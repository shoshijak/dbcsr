...
//===========================================================================
void print_matrix_(int n_a, int m, int n, double* mat_trs_a){
    int index = 0;
    for(int s=0; s < n_a; s++){
        printf("\t[s=%i]\n", s);
...
inline void validate_transpose_kernel(ACC_DRV(function)& kern_func, int threads, ACC_DRV(stream) stream, int m, int n){
    ...
    int n_blocks = h->n_stack_trs_a * num_tiles;
    printf("[%i,%i](val) offset=%i, stack_size=%i, num_tiles=%i, num_blocks=%i, threads=%i\n",m, n, 0, h->n_stack_trs_a, num_tiles, n_blocks, threads);
    launch_kernel_from_handle(kern_func, n_blocks, threads, stream, args);
