...
def check_autotuning_data_path(autotuning_data_path):
    # sanity checks
    assert os.path.exists(autotuning_data_path),      "This path does not exist: " + autotuning_data_path
    assert len(os.listdir(autotuning_data_path)) > 0, "No folders found in path: " + autotuning_data_path
    # print infos
    print("Number of tuning data folders found: {}".format(len(os.listdir(autotuning_data_path))))
...
def get_algorithm_to_explore(algo):
    algo_to_read = [algo] if algo != 'all' else ['tiny', 'small', 'medium', 'largeDB1', 'largeDB2']
...
def get_files_to_read(folders_to_read, algo_to_read):
    ...
    for i, kernel_folder in enumerate(folders_to_read):
        print('\nfrom {}, read                                  ({}/{:,})'.format(kernel_folder, i+1, len(folders_to_read)))
