...
extern "C" int acc_memcpy_d2d(const void *devmem_src, void *devmem_dst, size_t count, void* stream){
  ...
  if(stream == NULL){
      ...
      cErr = hipMemcpyAsync (devmem_dst, devmem_src, count, hipMemcpyDeviceToDevice, *custream);
  }
...
extern "C" int acc_memset_zero(void *dev_mem, size_t offset, size_t length, void* stream){
  ...
  if(stream == NULL){
      ...
      cErr = hipMemsetAsync ((void *) (((char *) dev_mem) + offset), (int) 0, length, *custream);
  }
